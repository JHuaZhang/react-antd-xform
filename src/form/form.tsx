...
import { composeValue, range, useHtmlIdPrefix } from './common-utils';
import {
  FormItemGroup,
  FormItemView,
  FormLayout,
...
import { Check, CheckConfig, Field, FormModel } from './model';
...
export const FormEnvProvider = ({
  children,
  ...override
}: FormEnvContextType & { children: React.ReactNode }) => {
  const parent = useFormEnv();
  return (
    <FormEnvContext.Provider value={{ ...parent, ...override }}>
      {children}
    </FormEnvContext.Provider>
  );
};
...
const FormModelConsumer = observer(
  ({ children }: React.ConsumerProps<FormModel<any>>) => {
    const model = useModel();
...
const FormArray = observer(
  <T extends any>({
    ...
    const arrayModel =
      arrayModelProp ??
      ((name === '&' ? parent : parent.getSubModel(name)) as FormModel<T[]>);
...
const FormObject = observer(
  ({
    ...
    const model = (
      name === '&' ? parent : parent.getSubModel(name)
    ) as FormModel;
